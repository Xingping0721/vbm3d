 ...
 **/
int run_vbm3d(
	const float sigma, Video<float> &vid_noisy, Video<float> &fflow, Video<float> &bflow, Video<float> &vid_basic, Video<float> &vid_denoised, const Parameters &prms_1, const Parameters &prms_2, const bool color_space)
{
	...
	cout << endl
		 << "Number of threads which will be used: " << nb_threads << endl;
#ifdef _OPENMP
			...
			if (prms_1.T_2D == DCT)
				for (unsigned n = 0; n < nb_threads; n++)
				{
					const unsigned nb_cols = ind_size(
						...
						(imCrops[n].ending_x == imCrops[n].source_sz.width) ? imCrops[n].ending_x - imCrops[n].origin_x - prms_1.k : imCrops[n].ending_x - imCrops[n].origin_x - prms_1.k - prms_1.n,
						prms_1.p);
			...
			if (prms_2.T_2D == DCT)
				for (unsigned n = 0; n < nb_threads; n++)
				{
					const unsigned nb_cols = ind_size(
						...
						(imCrops[n].ending_x == imCrops[n].source_sz.width) ? imCrops[n].ending_x - imCrops[n].origin_x - prms_2.k : imCrops[n].ending_x - imCrops[n].origin_x - prms_2.k - prms_2.n,
						prms_2.p);
...
inline void patch_trajectory(
	int px, int py, int pt, const VideoSize &sz, const Video<float> &fflow, const Video<float> &bflow, const Parameters &prms, vector<vector<int>> &trajectory)
{
	using std::max;
	using std::min;
	using std::round;
 ...
 **/
void vbm3d_1st_step(
	const float sigma, Video<float> const &vid_noisy, Video<float> &fflow, Video<float> &bflow, const Parameters &prms, fftwf_plan *plan_2d, fftwf_plan *plan_2d_inv, fftwf_plan *plan_1d, fftwf_plan *plan_1d_inv, VideoUtils::CropPosition *crop, const bool color_space, Video<float> &numerator, Video<float> &denominator)
{
	...
	if (crop == NULL)
		...
		ind_initialize(row_ind, (crop->origin_y == 0) ? 0 : prms.n,
					   (crop->ending_y == crop->source_sz.height) ? crop->ending_y - crop->origin_y - prms.k : crop->ending_y - crop->origin_y - prms.k - prms.n,
					   prms.p);
	...
	if (crop == NULL)
		...
		ind_initialize(column_ind, (crop->origin_x == 0) ? 0 : prms.n,
					   (crop->ending_x == crop->source_sz.width) ? crop->ending_x - crop->origin_x - prms.k : crop->ending_x - crop->origin_x - prms.k - prms.n,
					   prms.p);
		...
		for (unsigned ind_i = 0; ind_i < row_ind.size(); ind_i++)
		{
			...
			for (unsigned ind_j = 0; ind_j < column_ind.size(); ind_j++)
			{
				...
				for (unsigned c = 0; c < chnls; c++)
					for (unsigned n = 0; n < nSx_r; n++)
						for (unsigned k = 0; k < kHard_2_t; k++)
							group_3D[n + k * nSx_r + c * kHard_2_t * nSx_r] = table_2D[k + n * kHard_2_t + c * kHard_2_t * prms.N];
				...
				if (prms.T_3D == HADAMARD)
					ht_filtering_hadamard(group_3D, hadamard_tmp, nSx_r, prms.k, prms.kt, chnls, sigma_table, prms.lambda3D, weight_table);
				else
					ht_filtering_haar(group_3D, hadamard_tmp, nSx_r, prms.k, prms.kt, chnls, sigma_table, prms.lambda3D, weight_table);

				//! Inverse transform along the temporal dimension
				if (prms.kt > 1 && prms.T_2D != BIOR)
					temporal_inv_transform(group_3D, prms.k, prms.kt, chnls, nSx_r, prms.N, plan_1d_inv);
							...
							for (unsigned p = 0; p < prms.k; p++)
								for (unsigned q = 0; q < prms.k; q++)
								{
									numerator(patch_j_r + q, patch_i_r + p, patch_t_r + t, c) +=
										kaiser_window[p * prms.k + q] * wx_r_table[c + ind_j * chnls] * group_3D_table[p * prms.k + q + t * kHard_2 + n * kHard_2_t + c * kHard_2_t * nSx_r + dec];
 ...
 **/
void vbm3d_2nd_step(
	const float sigma, Video<float> const &vid_noisy, Video<float> const &vid_basic, Video<float> &fflow, Video<float> &bflow, const Parameters &prms, fftwf_plan *plan_2d, fftwf_plan *plan_2d_inv, fftwf_plan *plan_1d, fftwf_plan *plan_1d_inv, VideoUtils::CropPosition *crop, const bool color_space, Video<float> &numerator, Video<float> &denominator)
{
	VideoSize sz = vid_noisy.sz;
...
inline float patchDistance(
	unsigned patch1, unsigned patch2, const Video<float> &vid, Video<float> &fflow, const Parameters &prms, vector<vector<int>> &traj_patch1, vector<vector<int>> &traj_patch2)
{
		...
		for (unsigned hc = 0; hc < vid.sz.channels; ++hc)
			for (unsigned hy = 0; hy < sPx; hy++)
				for (unsigned hx = 0; hx < sPx; hx++)
					dist += (dif = (vid(px + hx, py + hy, pt + ht, hc) - vid(qx + hx, qy + hy, qt + ht, hc))) * dif;
...
inline void localSearch(
	unsigned pidx, unsigned rpidx, unsigned s, const Video<float> &vid, std::unordered_map<unsigned, int> &alreadySeen, std::vector<std::pair<float, unsigned>> &bestPatches, Video<float> &fflow, const Parameters &prms)
{
	int sWx = s;
	int sWy = s;
	int sPx = prms.k;
...
int computeSimilarPatches(
	std::vector<float> &output, std::vector<unsigned> &index, unsigned pidx, const Video<float> &vid, Video<float> &fflow, Video<float> &bflow, const Parameters &prms)
{
 ...
 **/
void dct_2d_process(
	vector<float> &DCT_table_2D, Video<float> const &vid, vector<unsigned> const &patch_table, fftwf_plan *plan, vector<float> const &coef_norm, Video<float> &fflow, const Parameters &prms)
{
	//! Declarations
...
void bior_2d_process(
	vector<float> &bior_table_2D, Video<float> const &vid, vector<unsigned> const &patch_table, vector<float> &lpd, vector<float> &hpd, Video<float> &fflow, const Parameters &prms)
{
	//! Declarations
	const VideoSize sz = vid.sz;
	const unsigned kHW = prms.k;
...
void ht_filtering_hadamard(
	vector<float> &group_3D, vector<float> &tmp, const unsigned nSx_r, const unsigned kHard, const unsigned ktHard, const unsigned chnls, vector<float> const &sigma_table, const float lambdaHard3D, vector<float> &weight_table)
{
	//! Declarations
...
void wiener_filtering_hadamard(
	vector<float> &group_3D_vid, vector<float> &group_3D_est, vector<float> &tmp, const unsigned nSx_r, const unsigned kWien, const unsigned ktWien, const unsigned chnls, vector<float> const &sigma_table, vector<float> &weight_table)
{
	//! Declarations
	...
	//! Weight for aggregation
	for (unsigned c = 0; c < chnls; c++)
		weight_table[c] = (weight_table[c] > 0.0f ? 1.0f / (float)(sigma_table[c] * sigma_table[c] * weight_table[c]) : 1.0f);
}
 ...
 **/
void dct_2d_inv(
	vector<float> &group_3D_table, const unsigned kHW, const unsigned ktHW, const unsigned N, vector<float> const &coef_norm_inv, fftwf_plan *plan)
{
	//! Declarations
	const unsigned kHW_2 = kHW * kHW;
	const unsigned kHW_2_t = kHW_2 * ktHW;
...
void bior_2d_inv(
	vector<float> &group_3D_table, const unsigned kHW, const unsigned ktHW, vector<float> const &lpr, vector<float> const &hpr)
{
	//! Declarations
	const unsigned kHW_2 = kHW * kHW;
	const unsigned kHW_2_t = kHW_2 * ktHW;
...
void temporal_transform(
	std::vector<float> &group_3D, const unsigned kHW, const unsigned ktHW, const unsigned chnls, const unsigned nSx_r, const unsigned N, fftwf_plan *plan)
{
	//! Declarations
	const unsigned kHW_2 = kHW * kHW;
	const unsigned kHW_2_t = kHW_2 * ktHW;
